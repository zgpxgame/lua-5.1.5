...
--   (h) minimal variable management code to differentiate each type
...
----------------------------------------------------------------------]]
  ...
  --   localfunc(), localstat()
  --------------------------------------------------------------------
  function luaY:new_localvar(name)
    local bl = fs.bl
    local locallist
    if bl then
  ...
  --------------------------------------------------------------------
  function luaY:create_local(name)
    self:new_localvarstr(name)
  end
    ...
    if fs == nil then  -- no more levels?
      ...
      if v >= 0 then
        ...
        if var.k == "VGLOBAL" then
          -- handle global var processing here
        else  -- LOCAL or UPVAL
          var.k = "VUPVAL"
        end
      end--if v
  ...
  function luaY:singlevar(v)
    ...
    self:singlevaraux(fs, varname, v, 1)
    self:log("    singlevar(kind): '"..v.k.."'")
  end
    ...
    if tok ~= ")" then  -- is 'parlist' not empty?
      repeat
        ...
        if c == "..." then
          ...
          self:next()
        elseif c == "<name>" then
          self:new_localvar(self:str_checkname())
        else
...
----------------------------------------------------------------------]]
  ...
  -- * used in for_stat()
  --------------------------------------------------------------------
  function luaY:fornum(varname, line)
    -- fornum -> NAME = exp1, exp1 [, exp1] DO body
    self:new_localvar(varname)
    self:new_localvarstr("(for limit)")
    self:new_localvarstr("(for step)")
  ...
  -- * used in for_stat()
  --------------------------------------------------------------------
  function luaY:forlist(indexname)
    -- forlist -> NAME {, NAME} IN explist1 DO body
    self:log(">> forlist: begin")
    local e = {}
    self:new_localvarstr("(for generator)")
    self:new_localvarstr("(for state)")
...
----------------------------------------------------------------------]]
  ...
  function luaY:for_stat()
    ...
    self:next()  -- skip 'for'
    local varname = self:str_checkname()  -- first variable name
    local c = tok
    if c == "=" then
