...
local function loadlist(fname, flist)
  ...
  while true do
    ...
    if string.match(d, "^%s*$") or string.match(d, "^#") then
...
loadlist("files-lua-5.1.txt", files)
loadlist("files-yueliang-5.1.txt", files)

-- pull in personal scripts to test if user specifies "ALL"
if arg[1] == "ALL" then
  loadlist("files-other-5.1.txt", files)
end
...
dofile("../orig-5.1.2/lzio.lua")
dofile("../orig-5.1.2/llex.lua")
dofile("../orig-5.1.2/lopcodes.lua")
dofile("../orig-5.1.2/ldump.lua")
dofile("../orig-5.1.2/lcode.lua")
dofile("../orig-5.1.2/lparser.lua")
...
for i, filename in ipairs(files) do
  ...
  local zio = luaZ:init(make_getF(filename), nil)
  if not zio then
  ...
  --io.stdout:write("collectgarbage: "..x.."\n")
end
